...
contract CryptoKids {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Onlt the owner can add kids");
        _;
    }
    ...
    // deposit funds to contract, specificallhy to a kid's account
    ...
    // kid checks if able to withdrawl
    ...
    // withdrawl money
    function withdrawl(address payable walletAddress) payable public {
        uint i = getIndex(walletAddress);
        require(msg.sender == kids[i].walletAddress, "You must be the correct kid to withdraw");
